...
(`frontend/src/lib/api/mock.ts`)을 구현해뒀다. `NEXT_PUBLIC_USE_MOCK=0`으로
바꾸면 바로 연동된다.

| 엔드포인트 | 프론트 사용처 |
|---|---|
| `GET /api/admin/notifications` | 헤더 알림 벨(`components/layout/NotificationBell.tsx`) |
| `POST /api/admin/notifications/read` | 벨을 열 때 `until: readMarker`로 호출 |

- **구현 확인**:
  - 배지는 `unreadCount`로 그림 (`items.length`를 쓰지 않음)
  - `read` 호출의 `until`에는 응답의 `readMarker`를 그대로 사용 (클라이언트
    시계 미사용)
  - 폴링 45초 간격 (`refetchInterval`), push 없음
  - `L`(임원) 이상에게만 벨이 보임 (`isLeaderOrAbove` 재사용)
  - mock에 22건을 넣어 "최근 20건 초과 시 배지 20+" 동작을 화면에서 확인함
- **미확정 사항**: 없음 — 계약 그대로 구현했다.
- **관련**: Jira 관련 티켓 없음(BE PR `feat/be-newcomer-notification`,
  `DECISIONS.md` 2026-09-09), FE PR 없음(아직 커밋 전)

---

## 2026-09-10 — 조직도 기능: 신규 데이터 모델·API 필요 (Jira `LIGHT-301`)

**상태**: 신규 요구사항 — `docs/spec/SPEC_API.md`에 아직 계약 없음.

- **내용**: 조직도 화면(Jira `LIGHT-300`[FE])에 필요한 조직 구조.
  - **임원단**
  - **마을 (9개)**: 촌장 1명 + 마을원 다수. 모든 회원은 마을에 **필수 소속**
    (1인 1마을) — 회원-마을 1:N 관계로 이미 있는 마을 스키마와 정합성 확인
    필요.
  - **팀 (6개, 지원제, 마을과 독립적인 별도 축)**: 찬양팀 · 중보기도팀 ·
    양육팀 · 심방팀 · 미디어팀 · 선교팀. 각 팀 구성은 팀장 / 총무 / 회계 /
    팀원. 팀 소속은 선택이며 마을과 무관 — 회원-팀 관계는 N:M으로 예상되나
    복수 팀 소속 허용 여부는 PM 미확정.
  - 팀 내 역할(팀장/총무/회계/팀원)을 표현할 방법 필요 — 마을의
    촌장/마을원 역할과 유사한 패턴으로 설계 가능.
- **왜 필요한지**: `/organization`(가칭) 조직도 화면.
- **미확정 사항**: 회원의 팀 복수 소속 허용 여부. 실제 조직도 데이터(임원/
  촌장/팀장 명단 등)는 PM이 기능 착수 시 별도 제공 예정 — 지금은 스키마/API
  설계만 필요.
- **관련 PR**: 없음 (아직 FE 구현 착수 전, Jira 티켓만 생성)

---

## 2026-09-04 (3) — 실제 주보에 **발행 번호(ISSUE 34)**가 있습니다. 계약에는 없습니다

실제 LIGHT 주보를 받아 목업에 넣으면서 확인한 것입니다. **지금 당장 고칠 것은
없고**, 계약을 언제 손댈지 판단만 부탁드립니다.

주보 표지 상단에 이렇게 찍혀 있습니다:
...
그런데 `GET /api/bulletins`(§5.2)와 `§5.1`의 주보는 **`serviceDate`만** 갖고
있어서(FE 타입 `types/api.ts`의 `Bulletin`·`BulletinSummary`), 화면이 주보를
부를 이름이 날짜뿐입니다.

**교회 안에서는 "34호"라고 부릅니다.** 지난 주보 목록에서 `2026-08-23`보다
`34호 (8/23)`가 사람이 찾기 쉽고, 연말에 "올해 몇 호까지 나왔나"도 셀 수 있습니다.

### 판단이 필요한 것

1. `issueNo`(그리고 `issueYear`)를 계약에 넣을 가치가 있는지
2. 넣는다면 **누가 채우는지** — 업로드 폼에 입력란을 두는 것과, 서버가 그 해
   주보 개수로 자동 부여하는 것 중 어느 쪽인지. 후자는 **중간에 한 주 빠지면
   교회가 쓰는 번호와 어긋납니다** (예: 8/16 주보를 안 올리면 8/23이 33호가 된다)
3. 넣지 않는다면 그대로 날짜로 갑니다 — 저희는 상관없습니다

`serviceDate`가 이미 사실상 키(§5.4 `DUPLICATE` 판정 기준)라서, `issueNo`는
표시용 부가 필드로만 생각했습니다. 키를 바꾸자는 뜻은 아닙니다.

### 덧 — 저희 쪽에서 함께 고친 것 (FE 내부, BE 영향 없음)

목업 주보 날짜가 **셋 다 주일이 아니었습니다**(08-10 / 08-17 / 08-24).
실물이 8월 23일이고 본문에 "다음 주일(8/30)"이 있어 8월 주일은 2·9·16·23·30일
입니다 → 08-09 / 08-16 / 08-23으로 고쳤습니다. mock 데이터라 계약과 무관합니다.

또한 실물이 **가로형(1231×875)**입니다. 스펙의 "장변 2048px"보다 작아서
업스케일하지 않고 원본 크기로 뒀습니다 — `pages[].width/height`를 계약대로
내려주시면 뷰어가 그대로 따라갑니다(지금까지 세로형만 있어서 가로형 CLS 예약이
검증된 적이 없었는데, 이번에 검증됐습니다).

---

## 2026-09-04 (2) — 주보 `M` 전환 **FE 대응 완료**. `develop`에 올리셔도 됩니다

> ✅ 전달됨 (2026-09-04) — 이슈 #169 댓글로 회신했다.

전달 주신 4곳 전부 고쳤습니다 (PR #170). **`develop`에 올리셔도 화면이 깨지지
않습니다.**

| # | 고친 곳 | 내용 |
|---|---|---|
| 1 | `app/bulletin/page.tsx` | `MemberGate` 적용 (`/documents` 패턴). 제목은 게이트 밖에 뒀습니다 — 안에 넣으면 로그인 유도 화면에 무슨 화면인지가 안 남습니다 |
| 2 | `lib/api/mock.ts` | `latest`·`list`·`get` 세 곳에 `requireSession()` — 지적하신 대로입니다 |
| 3 | `components/layout/navigation.ts` | 주보를 공개 메뉴 → **자료 그룹**으로 이동 |
| 4 | `app/home/page.tsx` | "▸ 주보 보기"를 **로그인한 사람에게만** 그립니다 (`_components/BulletinLink.tsx`) |

2026-08-25의 공개 열람 전환을 근거로 달려 있던 주석은 네 곳 모두 갱신했습니다.

**실측**(mock): 비로그인 → 게이트 + 로그인 버튼(뷰어 내용 유출 없음) ·
회원 → 뷰어·지난 주보 목록 정상 · 비로그인 홈에 주보 링크 없음 ·
로그인 홈에는 있음. `tsc --noEmit`·`lint` 통과.

### 회신 ①: `thumbUrl` — **계약은 그대로 두겠습니다**

> ✅ 전달됨 (2026-09-04) — 이슈 #169 댓글로 회신했다.

목록은 이미 48×64px + `loading="lazy"`로 그리고 있어 요청하신 두 가지는
충족된 상태입니다. 주보는 주 1회씩 쌓여 목록이 짧으니 **지금은 재가공 없이
갑니다.** 목록이 길어져 부담되면 그때 "업로드 시 축소본도 함께" 쪽으로
바꾸겠습니다 — 그 판단이 필요해지면 저희가 먼저 말씀드리겠습니다.

다만 **이름이 내용을 속이고 있어서**(썸네일이 아니라 1쪽 원본) `types/api.ts`
주석에 그 사실을 적어뒀습니다. 계약 필드명을 바꾸자는 뜻은 아닙니다.

### 회신 ②: 장별 다운로드 `GET /api/bulletins/{id}/pages/{pageNo}/download`

> ✅ 전달됨 (2026-09-04) — 이슈 #169 댓글로 회신했다.

**권한 `M`, 경로 제안 그대로 좋습니다.** 사진첩(§6)에 함께 붙여주세요.

### 남은 것 (이 건과 무관하지만 함께 봐주시면 좋겠습니다)

바로 아래 2026-09-04 (1) 항목의 출석부 집계 모수 불일치
(`checkedCount` vs `rosterCount`) — 아직 회신 전입니다.
...
## 2026-09-01 — develop 통합 실측 완료 — **FE·BE가 붙습니다.** 확인 요청 2건 추가

**상태**: `develop`(`449d00e`)을 워크트리로 떼서 **BE(8080) + FE(3001)를 실제로
띄우고 FE의 `/api` 프록시를 통해** 관통했습니다. 로컬 Postgres 빈 DB에 V1→V4가
순서대로 적용됐고, 명단 CSV 4행이 한글 안 깨지고 임포트됐습니다.
**계약 불일치 0건 · 양쪽 서버 로그에 에러 0건.** 수고하셨습니다.

### 통과한 것 (19항목)

| 항목 | 결과 |
|---|---|
| verify-roster 정상 (하이픈 없는 전화번호) | `200` `{registrationToken, name, expiresIn:300}` |
| 이름 불일치 · 명단 없음 · 접미사 누락 | **셋 다 `401` + 완전히 같은 문구** |
| `김도연b` 접미사 포함 | `200` — 접미사가 이름의 일부로 동작 |
| register | `201` `{id,name,role}` · **Set-Cookie 없음** |
| registrationToken 재사용 | `401` (1회용) |
| login | `200` + access/refresh 쿠키 · `/auth/me` 필드 일치 |
| 없는 아이디 · 틀린 비밀번호 · 5회 후 잠금 | **전부 같은 401.** 잠긴 뒤엔 올바른 비밀번호도 401 |
| 한글 25자(75바이트) 비밀번호 | `500`이 아니라 `400` |
| 리셋 코드 (소문자·하이픈 없이 입력) | `204` → 재사용 `401` → 새 비밀번호 로그인 `200` |
| 비밀번호 변경 | `204` + 이 기기 쿠키 재발급 · 현재 비밀번호 틀리면 **400** |
| 마지막 전도사 탈퇴 | `400`으로 막힘 |
| 카카오 전용 계정 탈퇴 (body 없이) | `204` + 명단 `claimed_by` 재개방 |
| 인가 매트릭스 | 예산안 익명 `403`/전도사 `200` · 회의록 익명 `401`/전도사 `200` · 공개공지 익명 `200` |

### 확인 요청 ③ 스펙 예시 비밀번호가 자기 규칙을 어깁니다

`SPEC_API §2.2`·`§2.3`의 예시 `"비밀번호12!"`는 **7자**라 `min(8)`에 걸립니다.
실제로 이 값을 그대로 써서 첫 시도가 400으로 떨어졌습니다. BE 테스트나 FE 문서가
이걸 복사하면 같은 데서 막힙니다 — 8자 이상 값으로 바꿔 주세요.

### 확인 요청 ④ `GET /bulletins/latest`가 익명에게 401입니다

`§10` 매트릭스는 `200 | 200 | 200 | 200`(공개)입니다. 인증 상태에서 404가 나오는
것을 보면 아직 미구현(M3 범위)이라 지금 당장 문제는 아닙니다. 다만 **구현할 때
SecurityConfig의 익명 허용을 같이 넣어야** 매트릭스와 맞습니다.
`/meetings`·`/albums`도 같은 상태입니다(익명 401 / 인증 404 = 미구현).
