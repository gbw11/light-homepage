...
| **Render** | 백엔드 | 750 인스턴스시간/월 | 결제 수단 미등록 | 서비스 1개만 · 핑 18h/일(558h) · `-Xmx400m` | ✅ 가동 |
| **Vercel** | 프론트엔드 | Hobby | 결제 수단 미등록 | 정적 우선 설계 | ✅ 가동 |
| **cron-job.org** | 슬립 방지 핑 | 무료 | 결제 개념 없음 | `*/10 6-23 * * *` (§3.5) | ✅ **가동 (2026-08-27 검증)** |
...
- ✅ **슬립 방지 핑이 실제로 동작한다** — 2026-08-27, **23분간 아무 요청도 보내지
  않은 뒤** 단일 측정에서 `200 / 0.246초`. 슬립 임계(15분)를 넘겼는데도 웜이었으므로
  핑이 깨워두고 있다는 뜻이다.
