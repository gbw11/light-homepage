...
R2는 **연산 횟수**에도 한도가 있다 — Class A(쓰기) 100만/월 · Class B(읽기) 1,000만/월. 초과분은 Class A가 100만 회당 $4.50, Class B가 $0.36다.

★ **presigned URL "발급"은 과금되지 않는다.** 서명은 우리 서버의 암호 연산이고 R2를 호출하지 않는다 — 과금은 **그 URL로 실제 요청이 갈 때** 일어난다. 우리 동작을 대입하면 Class A는 **업로드(`PutObject`)뿐**이고, 조회(`GetObject`)는 Class B, **삭제(`DeleteObject`)는 무료**다. 사람이 사진을 올리는 행위라 월 100만 회는 사실상 도달하지 않는다.

그래도 센다 (`r2_operation_counters`, V7). 위험한 것은 사용량이 아니라 재시도 루프처럼 **회원 수와 무관하게 늘어나는 사고**이고, 그것을 청구서가 아니라 우리 로그에서 먼저 보기 위한 것이다 — 과금 차단 장치가 아니라 **버그 조기 발견 장치**다.

⚠️ **R2는 무료 한도가 있어도 활성화에 결제 수단을 요구한다.** 그러면 "결제 수단이 없으므로 과금이 불가능하다"는 1차 방어가 성립하지 않는다. **2026-09-03에 A안(R2 사용 + 2차 가드 강화)으로 결정하고 카드를 등록했다** — 경위와 선택지는 [`COST_GUARDRAILS.md §3.6`](../ops/COST_GUARDRAILS.md).

⚠️ **그래서 이제 과금을 막는 것은 우리 코드뿐이다.** 특히 Class A(쓰기·presigned URL 발급)를 세는 장치를 M3 착수와 함께 넣는다 — 저장 용량 가드만으로는 그쪽이 안 막힌다.
...
| 5 | 드림센터 길찾기 사진 3장 | 약도 ✅(`RouteMap`) / 외관·입구 플레이스홀더 | M1 (촬영 필요) |
