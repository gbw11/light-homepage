...
/** "8/19 가입" (WIREFRAME.md §19) — 연도는 목록에서 노이즈라 뺀다 */
...
/**
 * WIREFRAME.md §19 — 회원 관리 `/admin/members` (FR-ADM-02/03/04/08).
 *
 * ~~승인 대기 탭~~ 은 2026-08-31 승인 폐지로 사라졌다 (SPEC_API §8 v1.3) —
 * 가입 즉시 MEMBER이므로 목록은 전체 하나뿐이다.
 *
...
export function MemberBoard() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: () => api.admin.members({ q: q || undefined }),
  });
...
function QueryStates({ isLoading, error }: { isLoading: boolean; error: unknown }) {
  if (isLoading) {
