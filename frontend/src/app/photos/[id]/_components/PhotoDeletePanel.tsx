 ...
 *
 * ## 왜 라이트박스 안인가 (그리드가 아니라)
 *
 * · **그리드에는 두지 않는다.** 무한 스크롤하는 썸네일 옆의 삭제 버튼은
 *   오탭 한 번이 곧 영구 삭제다.
 * · **대량 삭제를 만들지 않는다.** `§6.9`에는 대량 삭제 엔드포인트가 없어
 *   N번 순차 요청이 되고, 중간 실패 시 "몇 장은 지워지고 몇 장은 남은" 상태를
 *   사용자에게 설명해야 한다 — 되돌릴 수 없는 동작에서 만들면 안 되는
 *   상태다. 그래서 **1장씩만** 지운다.
 *   (선택 모드는 ZIP 다운로드 전용이었고, PM 결정 2026-08-25로 ZIP과 함께
 *   화면에서 사라졌다.)
